...
var inputFile = flag.String("inputFile", "input", "Relative file path to use as input.")
...
func main() {
	...
	fmt.Sprintf("%v * %v = %v", ones, threes, ones * threes)
	...
	for i := len(voltages) - 2; i >= 0 ; i-- {
		...
		for diff := 1; diff <= 3 ; diff++ {
			fmt.Println(i, voltages[i])
			i2 := voltages[i] + diff
			fmt.Println(i2)
			if pos, ok := exists[i2] ; ok{
				sum += ways[pos]
			}
			fmt.Println(sum)
			fmt.Println("*********")
...
func recur(index int, voltages []int) int {
	curr := voltages[index]
	//if index == 0 {
	//	return 0;
	//}
	if curr - voltages[index] > 3 {
		return 0
	}
	if curr == voltages[index] {
		return 1
	} else if curr - voltages[index] <= 3 {
		return 1
	}
 	return recur(index - 1, voltages)
}



//*
//Advent of Code[About][Events][Shop][Settings][Log Out]dodg3this 20*
...
//
//Your puzzle answer was 1322306994176.
//
